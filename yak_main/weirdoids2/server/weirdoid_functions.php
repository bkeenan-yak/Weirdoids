...
function getWeirdoid($user_weirdoid_id)
{
	...
	$imgroot = $docroot."/weirdoids/user_images/";
	$imgname = "../weirdoids/user_images/".$user_weirdoid_id.".jpg";
	
	//echo 'imgname = ' . $imgname;
	echo '--FILE__' . dirname(__FILE__) . '<br>';
	...
	if (mysql_num_rows($result) == 0) {
		$istatus["errorcode"] = 0;
		$istatus["errormsg"] = "No data found for user weirdoid";
...
function getWeirdoids($userid,$target_id)
{
	...
	if ($target_id != $userid)
	{
		...
		$qry=sprintf("SELECT w.user_id, yaklogin, w.user_weirdoid_id, w.fname, u.avatar,
				...
				and up.post_type_id = 1
				and u.user_id = %d
				ORDER BY w.created desc, w.user_weirdoid_id, s.cyclename", $target_id);
	}
	else {
...
function getPostComments($userpostid) {
	...
	$qry=sprintf("SELECT DISTINCT u.yaklogin, p.user_post_id, sc.comment_text, p.created, sc.has_placeholder
...
echo "drawImage ". $head_png . " " . $body_png . '<br>';;
